...
class ProjectGenerator {
  ...
  Future<void> create() async {
    ...
    logger.info('  - Refer to https://pub.dev/packages/secret_key_scrypt_generator for generating secretkey.dart file');
    if (config.useFirebase) {
      logger.info('\n 🔥 Firebase setup:');
      logger.info('  - Place your google-services.json file at: android/app/google-services.json');
      logger.info('  - Place your GoogleService-Info.plist file at: ios/Runner/GoogleService-Info.plist');
      logger.info('  - or use flutterfire CLI to configure Firebase automatically');
      logger.info('  - For more details, refer to the Firebase setup guide in the README.md file');
    }
    logger.info('\n 🔄 Sync freezed and other Build Runners');
    logger.info('   - Make sure the code was clear (No errors)');
    logger.info('   - Run task with Cmd/Ctrl + Shift + P → Tasks: Run Task -> Synchronize Build Runners');
    ...
    logger.info('  3. Under "Configurations", duplicate "Debug", "Profile", and "Release" for each flavor: development, staging');
    logger.info('     (e.g., Debug-development, Release-development, etc.)');
    logger.info('  4. Rename origin configurations intu "Debug-production", "Profile-production", and "Release-production"');
    logger.info('  5. Go to "Build Settings" in TARGETS → Runner');
    logger.info('  6. Search for "Product Bundle Identifier"');
    logger.info('  7. Set the bundle identifier for each configuration accordingly');
    logger.info('     (e.g., com.example.app.development, com.example.app.staging, etc.)');
    logger.info('  8. Setup display name for each configuration environment if needed.');
    logger.info('     - Tap Add icon to add new User-Defined Setting');
    logger.info('     - Set "APP_DISPLAY_NAME" as the key and your desired app name as the value for each configuration');
    logger.info('     - e.g., "Test App Dev" for development, "Test App Staging" for staging, etc.');
    logger.info('     - Open Info.plist and set the Bundle display name to \$(APP_DISPLAY_NAME)');
    logger.info('');
    logger.info(' 📃 This project uses an iOS Firebase script located at: ios/firebase.sh');
    logger.info('  Please complete the following steps in Xcode to complete the iOS flavoring setup:');
    ...
    logger.info('  6. Make sure this phase runs BEFORE "Copy Bundle Resources"');
    logger.info('  7. Create schemes: development, staging, production');
    logger.info('     (Product → Scheme → Manage Schemes)');

    logger.info('\n ▶ Run app:');
    logger.info('  flutter run --flavor development -t lib/main_development.dart');
  }
  ...
  Future<ProjectConfig> _collectInputsCreateProject() async {
    ...
    final firebaseInput = Input(prompt: 'Use Firebase? (Y/n): ').interact().trim();
    final useFirebase = firebaseInput.isEmpty || firebaseInput.toLowerCase() == 'y' || firebaseInput.toLowerCase() == 'yes';
    ...
    return ProjectConfig(
      ...
      stateManagement: stateManagementIndex == 0 ? StateManagement.getx : StateManagement.riverpod,
    );
  }
